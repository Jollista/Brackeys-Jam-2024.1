...
func _process(delta):
	...
	if nav_agent.is_navigation_finished():
		print(name, " finished navigation")
		return
...
func move_to_point(delta, speed):
	...
	print("PC target_pos = ", target_pos)
	var direction = global_position.direction_to(target_pos)
	face_direction(target_pos)
	#print("setting velocity")
	print("direction is ", direction)
	print("speed is ", speed)
	print("direction * speed is ", direction * speed)
...
func _input(event):
	...
	if my_turn and Input.is_action_just_pressed("left_mouse"):
		...
		if result.has("position"):
			...
			print("result position ", result.position, " which should equal ", nav_agent.target_position)
		
		print(name, " positioned at ", global_position, " or ", position)
